...
import { listenVenuesByStatus, setVenueStatus } from '../../firebase/adminService';
            ...
            {pending.map((v) => (
              <View key={v.id} style={styles.pendingCard}>
                <View style={styles.pendingRow}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={styles.venueName}>{v.name?.trim() || 'متجر بدون اسم بعد'}</Text>
                    <Text style={styles.venueMeta}>{v.type?.trim() || 'نوع غير محدد'}</Text>
                  </View>
                ...
                <View style={styles.actionsRow}>
                  <Pressable onPress={() => approve(v)} style={styles.approveBtn}>
                    <Text style={styles.approveText}>اعتماد ونشر</Text>
                  </Pressable>
                  <Pressable onPress={() => reject(v)} style={styles.rejectBtn}>
...
const styles = StyleSheet.create({
  ...
  email: { fontFamily: brandFont.enRegular, fontSize: 11.5, color: brandColors.white55, writingDirection: 'ltr', marginTop: 4 },
  ...
  statRow: { flexDirection: 'row', gap: 9, backgroundColor: brandColors.adminBg, paddingHorizontal: 20, paddingBottom: 22 },
  ...
  pendingCard: { backgroundColor: '#fff', borderWidth: 1, borderColor: brandColors.border09, borderRadius: 24, padding: 14 },
  ...
  venueName: { fontFamily: brandFont.arExtraBold, fontSize: 15.5, color: brandColors.text },
  venueMeta: { fontFamily: brandFont.arRegular, fontSize: 11.5, color: brandColors.ink55, marginTop: 3 },
